...
def measure_performance_with_memory(sort_function, data):
    ...
    for name, arr in data.items():
        copied_arr = arr[
            :
        ]  # Make a shallow copy to preserve the original for in-place sorting
        ...
        memories[name] = (
            memory_used[0] - memory_used[1]
        )  # Subtract initial memory from peak memory
...
def print_results_with_memory(results):
    for sort_method, details in results.items():
        ...
        for arr_type, metrics in details.items():
            ...
            try:
                print(
                    f"{arr_type} array: Time: {time_taken:.6f} seconds, Memory: {memory_used:.2f} MiB"
                )
            except Exception:
...
def do_performance_test_with_memory(base_sample_size, num_test_sets):
    ...
    for test_iteration in range(1, num_test_sets + 1):
        ...
        t1 = time.time()
        print(
            f"#### test {test_iteration}: sample size {sample_size} ###########################################################"
        )
        print()
        test_results.extend(
            test_collect_results_with_memory(generate_arrays(sample_size))
        )
        print(f"Iteration {test_iteration} run time: {time.time()-t1}")
        print()

    print(
        f"Total test duration with memory analysis for base sample size {base_sample_size} and doubling every iteration for {num_test_sets} times: {time.time()-test_t1}s"
    )
    return test_results
...
def do_performance_test(base_sample_size, num_test_sets, iteration_size_factor=2):
    ...
    print(
        f"Total test duration for base sample size {base_sample_size} and doubling every iteration for {num_test_sets} times: {time.time()-test_t1}s"
    )
    return test_results
